...
public class Minimap : Actor
{
    ...
    [SerializeField] private Image target;               //ターゲットのUIイメージ
    //[SerializeField] private Image target;               //対象のUIイメージ

    //メモ
    //プレイヤーと対象のx座標、z座標をそれぞれ求めフィールドの幅、高さでそれぞれ割ると-1から1の範囲で表現することができるのでそれを利用する
    ...
    void Update()
    {
        ...
        //target.rectTransform.position = GetMinimapPos(RespawTarget.GetCurrentTargetObj().transform.position);

        //if (GameManager.Instance.skillManager.GetTargetMinimapFlag())
        //{
        //    target.enabled = true;
        //}
        //else
    ...
    private Vector2 GetMinimapPos(Vector3 pos)
    {
        ...
        return (new Vector2(pos.x/* - fieldObj.transform.position.x*/, pos.z /*- fieldObj.transform.position.z*/) / (fieldParam / 2)) * (fieldParam / 2);// + (Vector2)basePos.rectTransform.position;
    }
}
