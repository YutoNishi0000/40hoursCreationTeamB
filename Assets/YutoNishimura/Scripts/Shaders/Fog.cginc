...
float GetForHeightFogParameter(float3 objectPos, float3 cameraPos, float densityY0, float densityAttenuation)
{
	...
	// sで微分すると、y座標に応じた減衰量が得られる
	// dc / ds = -c * d0 * exp(-k * y);
	// ここでyが邪魔なのでyをsで表す
	// y = y0 + (v.y / |v| * s);
	// よってsが|v|になれば、分子分母が消えてv.yが残り、これをy0に足せばカメラの高さになる
	...
	//これはまとめると指定した高さになるまでループして減衰されたカラー情報を返すことになる
...
float GetForHeightFogParameter2(float3 objectPos, float3 cameraPos, float densityY0, float densityAttenuation, float fogHeight)
{
	...
	float3 camToObjVec = cameraPos - objectPos;      //カメラとオブジェクト間のベクトル
	float ret;
	float tmp = densityY0 * exp(-densityAttenuation * objectPos.y);   //上記で言うA（定数）
	if (camToObjVec.y == 0.0) // 単純な均一フォグ
	{
		ret = exp(-tmp);
	}
	...
	{
		float kvy = densityAttenuation * fogHeight;   //(k * v.y)
		ret = exp(tmp / kvy * (exp(-kvy) - 1.0));         //c = exp(d0 * |v| / (k * v.y) * exp(-k * y0) * (exp(-k * v.y) - 1))
	}
	return ret;
}